...
function extrairBatidasEmColunas(pontos: Ponto[], hasAtestado: boolean) {
...
function buildAllRows(pontos: Ponto[], ajustes: AjusteBanco[], config?: Config): RelatorioRow[] {
  ...
  let rows = diasTodasKeys.map((dia) => {
    ...
    const metaMinutos = hasAtestado && pts.length === 0 ? 0 : metaBase;
    const saldoMinutos = trabalhadoMinutos - metaMinutos;

    const colBatidas = extrairBatidasEmColunas(pts, hasAtestado);
  ...
  if (OCULTAR_DIAS_SEM_ATIVIDADE) {
    rows = rows.filter((r) => r.metaMinutos > 0 || r.trabalhadoMinutos > 0 || r.hasAtestado);
  }
...
export async function gerarRelatorioExcel(
  ...
  const rows = buildAllRows(pontos, ajustes, config);
...
export function gerarRelatorioCSV(pontos: Ponto[], ajustes: AjusteBanco[], saldoTotal: string, config?: Config) {
  const rows = buildAllRows(pontos, ajustes, config);
  const lines: string[] = [];
