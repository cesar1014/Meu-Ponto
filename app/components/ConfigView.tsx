...
function JornadaView({
  ...
  const DayInput = ({ day }: { day: { key: keyof DailyTargets; label: string } }) => {
    const split = splitMinutes(config.dailyTargets[day.key] ?? 0);
    return (
      <div className="rounded-2xl border p-3" style={{ borderColor: 'var(--border)', background: 'var(--card2)' }}>
        ...
        <div className="mt-2 flex items-center gap-2">
          <input
            ...
            max={24}
            value={split.h}
            onChange={(e) => updateDaily(day.key, Math.max(0, Number(e.target.value)), split.m)}
          ...
          <input
            ...
            max={59}
            value={split.m}
            onChange={(e) => updateDaily(day.key, split.h, Math.max(0, Math.min(59, Number(e.target.value))))}
            className="w-14 rounded-xl border px-2 py-1 text-center text-sm outline-none"
            style={{ borderColor: 'var(--border)', background: 'var(--card)', color: 'var(--text)' }}
          />
