...
export default function AjustesPage() {
  ...
  const pontosDiaBase = useMemo(() => {
    ...
    TIPOS_PONTO.forEach((tipo) => {
      ...
      const normalized = time ? normalizeTimeValue(time) : null;
      if (normalized) list.push({ tipo, time: normalized });
    });
    pontosExtras.forEach((p) => {
      const normalized = p.time ? normalizeTimeValue(p.time) : null;
      if (normalized) list.push({ tipo: p.tipo, time: normalized });
    });
                  ...
                  {TIPOS_PONTO.map((tipo) => (
                    <div key={tipo} className="flex items-center gap-2">
                      ...
                      <TimeField
                        ...
                        onChange={(value) => setPontoEntradas({ ...pontoEntradas, [tipo]: value })}
